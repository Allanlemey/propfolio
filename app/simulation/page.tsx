...
import { PDFReport } from "@/components/simulation/pdf-report";
import {
  calcMonthlyPayment,
...
export default function SimulationPage() {
  ...
  async function handleActualDownload() {
    ...
    try {
      ...
      // Full rendered height in mm (keeping canvas aspect ratio at pageW)
      ...
      for (let i = 0; i < pageCount; i++) {
        if (i > 0) pdf.addPage();
        // Shift image up so the right slice sits at y=0 on each page
        pdf.addImage(imgData, "PNG", 0, -i * pageH, pageW, totalH);
      }

      const safeName = (simName || "simulation").replace(/[^a-z0-9]/gi, "_").toLowerCase();
